...
\@x{}\moduleLeftDash\@xx{ {\MODULE} CassandraPaxos}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers}%
\@pvspace{8.0pt}%
\@x{ Maximum ( S ) \.{\defeq}}%
\begin{lcom}{8.2}%
...
\@x{\@s{8.2} {\IF} S \.{=} \{ \} \.{\THEN} \.{-} 1}%
 ...
 \.{\geq} m}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS} Value ,\, Acceptor ,\, Quorum}%
 \@x{ {\ASSUME}\@s{4.1} \.{\land} \A\, Q \.{\in} Quorum \.{:} Q \.{\subseteq}
 Acceptor}%
 \@x{\@s{42.34} \.{\land} \A\, Q1 ,\, Q2 \.{\in} Quorum \.{:} Q1 \.{\cap} Q2
 \.{\neq} \{ \}}%
\@pvspace{8.0pt}%
\@x{ Ballot \.{\defeq} Nat}%
\@pvspace{8.0pt}%
...
\@pvspace{8.0pt}%
\@x{ Message \.{\defeq}}%
\@x{\@s{28.7} [ type \.{:} \{\@w{Prepare} \} ,\, bal\@s{3.18} \.{:} Ballot ]}%
 \@x{\@s{8.2} \.{\cup}\@s{9.38} [ type \.{:} \{\@w{Promise} \} ,\, acc \.{:}
 Acceptor ,\, bal\@s{21.61} \.{:} Ballot ,\,}%
 \@x{\@s{31.47} maxAccBal\@s{4.79} \.{:} Ballot \.{\cup} \{ \.{-} 1 \} ,\,
 maxAccVal\@s{4.83} \.{:} Value \.{\cup} \{ None \} ,\,}%
 \@x{\@s{31.47} macComBal \.{:} Ballot \.{\cup} \{ \.{-} 1 \} ,\, maxComVal
 \.{:} Value \.{\cup} \{ None \} ]}%
 \@x{\@s{8.2} \.{\cup}\@s{9.38} [ type \.{:} \{\@w{Propose} \} ,\, bal \.{:}
 Ballot ,\, val \.{:} Value ]}%
 \@x{\@s{8.2} \.{\cup}\@s{9.38} [ type \.{:} \{\@w{Accept} \} ,\, acc \.{:}
 Acceptor ,\, bal \.{:} Ballot ,\, val \.{:} Value ]}%
 \@x{\@s{8.2} \.{\cup}\@s{9.38} [ type \.{:} \{\@w{Commit} \} ,\, bal \.{:}
 Ballot ,\, val \.{:} Value ]}%
 \@x{\@s{8.2} \.{\cup}\@s{9.38} [ type \.{:} \{\@w{Ack} \} ,\, acc \.{:}
 Acceptor ,\, bal \.{:} Ballot ,\, val \.{:} Value ]}%
\@pvspace{8.0pt}%
 \@x{ {\VARIABLES} maxBal ,\, maxAccBal ,\, maxAccVal ,\, maxComBal ,\,
 maxComVal ,\, msgs}%
 \@x{ vars \.{\defeq} {\langle} maxBal ,\, maxAccBal ,\, maxAccVal ,\,
 maxComBal ,\, maxComVal ,\, msgs {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ TypeOK \.{\defeq} \.{\land} maxBal\@s{4.1} \.{\in} [ Acceptor
 \.{\rightarrow} Ballot \.{\cup} \{ \.{-} 1 \} ]}%
 \@x{\@s{56.14} \.{\land} maxAccBal \.{\in} [ Acceptor \.{\rightarrow}
 Ballot\@s{6.45} \.{\cup} \{ \.{-} 1 \} ]}%
 \@x{\@s{56.14} \.{\land} maxAccVal\@s{4.1} \.{\in} [ Acceptor \.{\rightarrow}
 Value\@s{4.83} \.{\cup} \{ None \} ]}%
 \@x{\@s{56.14} \.{\land} maxComBal \.{\in} [ Acceptor \.{\rightarrow}
 Ballot\@s{1.62} \.{\cup} \{ \.{-} 1 \} ]}%
 \@x{\@s{56.14} \.{\land} maxComVal\@s{4.1} \.{\in} [ Acceptor \.{\rightarrow}
 Value \.{\cup} \{ None \} ]}%
\@x{\@s{56.14} \.{\land} msgs \.{\subseteq} Message}%
\@pvspace{16.0pt}%
 \@x{ Init \.{\defeq} \.{\land} maxBal\@s{4.1} \.{=} [ a \.{\in} Acceptor
 \.{\mapsto} \.{-} 1 ]}%
 \@x{\@s{35.70} \.{\land} maxAccBal \.{=} [ a \.{\in} Acceptor \.{\mapsto}
 \.{-} 1 ]}%
 \@x{\@s{35.70} \.{\land} maxAccVal\@s{4.1} \.{=} [ a \.{\in} Acceptor
 \.{\mapsto} None ]}%
 \@x{\@s{35.70} \.{\land} maxComBal \.{=} [ a \.{\in} Acceptor \.{\mapsto}
 \.{-} 1 ]}%
 \@x{\@s{35.70} \.{\land} maxComVal\@s{4.1} \.{=} [ a \.{\in} Acceptor
 \.{\mapsto} None ]}%
\@x{\@s{35.70} \.{\land} msgs \.{=} \{ \}}%
\@pvspace{8.0pt}%
\@x{ Send ( m ) \.{\defeq} msgs \.{'} \.{=} msgs \.{\cup} \{ m \}}%
\@pvspace{16.0pt}%
 \@x{ Prepare ( b )\@s{3.07} \.{\defeq} \.{\land} Send ( [ type
 \.{\mapsto}\@w{Prepare} ,\, bal \.{\mapsto} b ] )}%
 \@x{\@s{68.82} \.{\land} {\UNCHANGED} {\langle} maxBal ,\, maxAccBal ,\,
 maxAccVal ,\, maxComBal ,\,}%
\@x{\@s{138.52} maxComVal {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Promise ( a ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land} \E\, m \.{\in} msgs \.{:}}%
\@x{\@s{26.53} \.{\land} m . type \.{=}\@w{Prepare}}%
\@x{\@s{26.53} \.{\land} m . bal \.{>} maxBal [ a ]}%
 \@x{\@s{26.53} \.{\land} maxBal \.{'} \.{=} [ maxBal {\EXCEPT} {\bang} [ a ]
 \.{=} m . bal ]}%
 \@x{\@s{26.53} \.{\land} Send ( [ type \.{\mapsto}\@w{Promise} ,\, acc
 \.{\mapsto} a ,\, bal \.{\mapsto} m . bal ,\,}%
 \@x{\@s{70.39} maxAccBal\@s{4.83} \.{\mapsto} maxAccBal [ a ] ,\,
 maxAccVal\@s{9.66} \.{\mapsto} maxAccVal [ a ] ,\,}%
 \@x{\@s{70.39} maxComBal \.{\mapsto} maxComBal [ a ] ,\, maxComVal
 \.{\mapsto} maxComVal [ a ] ] )}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} maxAccBal ,\, maxAccVal ,\,
 maxComBal ,\, maxComVal {\rangle}}%
\@pvspace{16.0pt}%
 \@x{ Propose ( b ,\, v ) \.{\defeq} \.{\land} {\lnot} \E\, m \.{\in} msgs
 \.{:} m . type \.{=}\@w{Propose} \.{\land} m . bal \.{=} b}%
\@x{\@s{78.43} \.{\land} \E\, Q \.{\in} Quorum \.{:}}%
 \@x{\@s{93.64} \.{\LET} Qmset \.{\defeq} \{ m \.{\in} msgs \.{:} \.{\land} m
 . type \.{=}\@w{Promise}}%
\@x{\@s{219.12} \.{\land} m . acc \.{\in} Q}%
\@x{\@s{219.12} \.{\land} m . bal\@s{1.57} \.{=} b \}}%
 \@x{\@s{114.04} maxAbal \.{\defeq} Maximum ( \{ m . maxAccBal\@s{4.83} \.{:}
 m \.{\in} Qmset \} )}%
 \@x{\@s{114.04} maxCbal\@s{0.02} \.{\defeq} Maximum ( \{ m . maxComBal \.{:}
 m \.{\in} Qmset \} )}%
\@x{\@s{118.14} val \.{\defeq} {\IF} maxAbal \.{>} maxCbal}%
 \@x{\@s{162.49} \.{\THEN} ( {\CHOOSE} m \.{\in} Qmset \.{:} m . maxAccBal
 \.{=} maxAbal ) . maxAccVal}%
\@x{\@s{162.49} \.{\ELSE} v}%
 \@x{\@s{93.64} \.{\IN}\@s{4.09} \.{\land} \A\, a \.{\in} Q \.{:} \E\, m
 \.{\in} Qmset \.{:} m . acc \.{=} a}%
 \@x{\@s{118.14} \.{\land} Send ( [ type \.{\mapsto}\@w{Propose} ,\, bal
 \.{\mapsto} b ,\, val \.{\mapsto} val ] )}%
 \@x{\@s{78.43} \.{\land} {\UNCHANGED} {\langle} maxBal ,\, maxAccBal ,\,
 maxAccVal ,\, maxComBal ,\, maxComVal {\rangle}}%
\@pvspace{24.0pt}%
 \@x{ Accept ( a ) \.{\defeq} \.{\land} \E\, m \.{\in} msgs \.{:} \.{\land} m
 . type \.{=}\@w{Propose}}%
\@x{\@s{132.34} \.{\land} maxBal [ a ] \.{\leq} m . bal}%
 \@x{\@s{132.34} \.{\land} maxBal \.{'} \.{=} [ maxBal {\EXCEPT} {\bang} [ a ]
 \.{=} m . bal ]}%
 \@x{\@s{132.34} \.{\land} maxAccBal \.{'} \.{=} [ maxAccBal {\EXCEPT} {\bang}
 [ a ] \.{=} m . bal ]}%
 \@x{\@s{132.34} \.{\land} maxAccVal \.{'}\@s{0.37} \.{=} [ maxAccVal
 {\EXCEPT} {\bang} [ a ]\@s{0.37} \.{=} m . val ]}%
 \@x{\@s{132.34} \.{\land} Send ( [ type \.{\mapsto}\@w{Accept} ,\, bal
 \.{\mapsto} m . bal ,\, val \.{\mapsto} m . val ,\,}%
\@x{\@s{172.11} acc \.{\mapsto} a ] )}%
 \@x{\@s{50.93} \.{\land} {\UNCHANGED} {\langle} maxComBal ,\, maxComVal
 {\rangle}}%
\@pvspace{16.0pt}%
 \@x{ Commit ( b ,\, v ) \.{\defeq} \.{\land} {\lnot} \E\, m \.{\in} msgs
 \.{:} m . type \.{=}\@w{Commit} \.{\land} m . bal \.{=} b}%
\@x{\@s{79.64} \.{\land} \E\, Q \.{\in} Quorum \.{:}}%
 \@x{\@s{94.85} \.{\LET} QAmset \.{\defeq} \{ m \.{\in} msgs \.{:} \.{\land} m
 . type \.{=}\@w{Accept}}%
\@x{\@s{219.18} \.{\land} m . acc \.{\in} Q}%
\@x{\@s{219.18} \.{\land} m . bal \.{=} b \}}%
 \@x{\@s{94.85} \.{\IN} \.{\land} \A\, a \.{\in} Q \.{:} \E\, m \.{\in} QAmset
 \.{:} m . acc \.{=} a}%
 \@x{\@s{79.64} \.{\land} Send ( [ type \.{\mapsto}\@w{Commit} ,\, bal
 \.{\mapsto} b ,\, val \.{\mapsto} v ] )}%
 \@x{\@s{79.64} \.{\land} {\UNCHANGED} {\langle} maxBal ,\, maxAccBal ,\,
 maxAccVal ,\, maxComBal ,\,}%
\@x{\@s{153.44} maxComVal {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ Ack ( a ) \.{\defeq} \.{\land} \E\, m \.{\in} msgs \.{:} \.{\land} m .
 type \.{=}\@w{Commit}}%
\@x{\@s{120.46} \.{\land} maxBal [ a ] \.{\leq} m . bal}%
 \@x{\@s{120.46} \.{\land} maxBal \.{'} \.{=} [ maxBal {\EXCEPT} {\bang} [ a ]
 \.{=} m . bal ]}%
 \@x{\@s{120.46} \.{\land} maxComBal \.{'} \.{=} [ maxComBal {\EXCEPT} {\bang}
 [ a ] \.{=} m . bal ]}%
 \@x{\@s{120.46} \.{\land} maxComVal \.{'}\@s{0.37} \.{=} [ maxComVal
 {\EXCEPT} {\bang} [ a ]\@s{0.37} \.{=} m . val ]}%
 \@x{\@s{120.46} \.{\land} Send ( [ type \.{\mapsto}\@w{Ack} ,\, bal
 \.{\mapsto} m . bal ,\, val \.{\mapsto} m . val ,\,}%
\@x{\@s{157.45} acc \.{\mapsto} a ] )}%
 \@x{\@s{49.74} \.{\land} {\UNCHANGED} {\langle} maxAccBal ,\, maxAccVal
 {\rangle}}%
\@pvspace{16.0pt}%
 \@x{ Next \.{\defeq} \.{\lor} \E\, b\@s{0.64} \.{\in} Ballot \.{:} \.{\lor}
 Prepare ( b )}%
 \@x{\@s{112.86} \.{\lor} \E\, v \.{\in} Value \.{:} Propose ( b ,\, v )
 \.{\lor} Commit ( b ,\, v )}%
 \@x{\@s{39.83} \.{\lor} \E\, a \.{\in} Acceptor \.{:} \.{\lor} Promise ( a )
 \.{\lor} Accept ( a ) \.{\lor} Ack ( a )}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Dec} 09 19:33:06 \ensuremath{CST} 2021 by \ensuremath{LENOVO
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Dec} 02
