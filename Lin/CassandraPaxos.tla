                ...
                IN  /\ \A a \in Q : \E m \in QRmset : m.acc = a 
                    /\ IF MeetCondition(balValue[b].expVal,maxValue) 
                            ...
                            ELSE /\ Send([type |-> "Terminate", bal |->b])
                                 /\ history' = Append(history,[type |-> "response", 
                                                   bal |-> b, flag |-> "FALSE", 
...
Ack(a) == /\ \E m \in msgs: /\ m.type="Commit"
          ...
          /\UNCHANGED <<maxAccBal, maxAccVal, dataResult, balValue,history>>
...
Check(seq, r) == IF seq = <<>> THEN TRUE 
                               ELSE LET cur  == Head(seq)
                                    IN \/ /\ cur.flag = “TRUE”
                                          /\ cur.oldVal = r
                                          /\ Check(Tail(seq), cur.newVal)
                                       \/ /\ cur.flag = “FALSE”
                                          /\ cur.oldVal # r
                                          /\ Check(Tail(seq), r)

FiniteSeq(S) == UNION {[1..n -> S]:n \in Cardinality(S)}
